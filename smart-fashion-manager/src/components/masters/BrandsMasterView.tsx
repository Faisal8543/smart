...
export const BrandsMasterView: React.FC = () => {
  const { brandsList, addBrand, updateBrand, deleteBrand } = useAppState();
  ...
  const openEditModal = (item: BrandMaster) => {
    setEditingItem(item);
    setName(item.name);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      updateBrand(editingItem.id, { name: name.trim(), status });
    } else {
      addBrand(name.trim(), status);
    }
    setModalOpen(false);
  };
  ...
  return (
    <div className="space-y-4" id="brands-master-container">
      {/* Toast alert */}
      {toast && (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-lg font-serif font-bold text-amber-200">Brand Master Directory</h2>
          <p className="text-xs text-slate-400">Configure designer clothing label brands.</p>
        </div>
        <button
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-12 text-center text-slate-500">
                    ...
                    No brands found. Click "New Brand" to begin.
                  </td>
                </tr>
              ) : (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/40">
              <h3 className="font-serif text-sm font-bold text-amber-200">
                {editingItem ? 'Edit Brand Record' : 'Create New Brand'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Cavalli & Co., Albin Mill..."
