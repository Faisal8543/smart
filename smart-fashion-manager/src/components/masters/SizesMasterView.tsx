...
import { SizeMaster } from '../../types';

export const SizesMasterView: React.FC = () => {
  const { sizesList, addSize, updateSize, deleteSize } = useAppState();
  const [searchTerm, setSearchTerm] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<SizeMaster | null>(null);
  ...
  // Search filter
  const filtered = sizesList.filter((s) =>
  ...
  const openEditModal = (item: SizeMaster) => {
    setEditingItem(item);
    setName(item.name);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      updateSize(editingItem.id, { name: name.trim(), status });
    } else {
      addSize(name.trim(), status);
    }
    setModalOpen(false);
  };
  ...
  return (
    <div className="space-y-4" id="sizes-master-container">
      {/* Toast alert */}
      {toast && (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-lg font-serif font-bold text-amber-200">Size Master Directory</h2>
          <p className="text-xs text-slate-400">Configure standardized size specifications.</p>
        </div>
        <button
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search sizes by spec..."
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={4} className="p-12 text-center text-slate-500">
                    ...
                    No sizes found. Click "New Size" to begin.
                  </td>
                </tr>
              ) : (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/40">
              <h3 className="font-serif text-sm font-bold text-amber-200">
                {editingItem ? 'Edit Size Record' : 'Create New Size'}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. S, M, L, XL, 38, 40, 42L..."
