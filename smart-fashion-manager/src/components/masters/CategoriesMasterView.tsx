...
export const CategoriesMasterView: React.FC = () => {
  const { categoriesList, addCategory, updateCategory, deleteCategory } = useAppState();
  ...
  const openEditModal = (item: CategoryMaster) => {
    setEditingItem(item);
    setName(item.name);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      updateCategory(editingItem.id, { name: name.trim(), status });
    } else {
      addCategory(name.trim(), status);
    }
    setModalOpen(false);
  };
  ...
  return (
    <div className="space-y-4" id="categories-master-container">
      {/* Toast alert */}
      {toast && (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-lg font-serif font-bold text-amber-200">Category Master Directory</h2>
          <p className="text-xs text-slate-400">Configure core product categorization tiers.</p>
        </div>
        <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/40">
              <h3 className="font-serif text-sm font-bold text-amber-200">
                {editingItem ? 'Edit Category Record' : 'Create New Category'}
              </h3>
              <button
