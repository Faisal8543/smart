...
export const PosView: React.FC = () => {
  ...
  const handleOpenUpiModal = async (
    ...
    try {
      ...
      // Notify Android customer display if available (local Windows ADB or cloud-safe fallback)
      fetch('/api/android/upi-display', {
        method: 'POST',
  ...
  // Dedicated Print Function using current document container
  const handlePrintInvoice = (invoiceToPrint?: Invoice | null) => {
    ...
    void printInCurrentDocument(printableHtml, `Invoice #${target.invoiceNo}`);
  };
  ...
  const executeProductsCheckout = () => {
    ...
    try {
      ...
      // Build complete standalone printable HTML and render in dedicated current-document print container
      const printableHtml = buildCompletePrintPageHtml(invoice, settings, invoices);
      void printInCurrentDocument(printableHtml, `Invoice #${invoice.invoiceNo}`);
  ...
  const executeFabricsCheckout = () => {
    ...
    try {
      ...
      // Populate printable receipt into dedicated current-document print container
      const printHtml = buildCompleteFabricPrintPageHtml(finalBill, settings);
      void printInCurrentDocument(printHtml, `Fabric Bill #${finalBill.billNo}`);
