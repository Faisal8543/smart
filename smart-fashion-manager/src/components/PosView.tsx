...
import { triggerWindowPrint } from '../utils/printWindowHelper';
...
export const PosView: React.FC = () => {
  ...
  const handlePrintInvoice = (invoiceToPrint?: Invoice | null) => {
    ...
    triggerWindowPrint(printWindow, printableHtml);
  };
  ...
  const executeProductsCheckout = () => {
    ...
    try {
      ...
      // Build complete standalone printable HTML and write directly into the print window
  ...
  const executeFabricsCheckout = () => {
    ...
    if (!printWindow) {
      showToast?.(
        'Pop-up blocked. Please allow pop-ups for Smart Fashion Manager to print receipt.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      // Populate printable receipt directly into the already-open window once
      const printHtml = buildCompleteFabricPrintPageHtml(finalBill, settings);
      triggerWindowPrint(printWindow, printHtml);
