...
export const ProductsView: React.FC = () => {
  ...
  const triggerPrintBarcode = () => {
    ...
    // 1. Immediately open top-level temporary print window synchronously inside click event
    ...
    try {
      ...
      // Populate using the SAME architecture as the working invoice print implementation
      ...
      console.error('Failed to prepare barcode print window:', err);
      if (printWindow && !printWindow.closed) {
        try {
          printWindow.close();
        } catch {}
      }
      showToast('Error preparing barcode print window.', 'error');
    } finally {
      setTimeout(() => {
