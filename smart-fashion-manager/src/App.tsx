...
const ShowroomAppContent: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState(() => {
    ...
    return allowedViews.includes(hash) ? hash : 'pos';
  });
  ...
  useEffect(() => {
    ...
    // Sync initial hash or default to pos
  ...
  // Ensure upon login success that destination defaults to POS Billing -> Products Bill
  useEffect(() => {
    if (isAuthenticated) {
      const rawHash = window.location.hash.slice(1);
      const hash = rawHash.split('/')[0];
